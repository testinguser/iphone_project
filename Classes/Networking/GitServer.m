...
NSString * const GitServerErrorDomain = @"GitServerErrorDomain";
...
// invocation on GitServer.
static void GitServerAcceptCallBack(CFSocketRef socket, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    GitServer *server = (GitServer *)info;
...
- (BOOL)start:(NSError **)error {
    ...
    _ipv4socket = CFSocketCreate(kCFAllocatorDefault, PF_INET, SOCK_STREAM, IPPROTO_Git, kCFSocketAcceptCallBack, (CFSocketCallBack)&GitServerAcceptCallBack, &socketCtxt);
	
    if (NULL == _ipv4socket) {
        if (error) *error = [[NSError alloc] initWithDomain:GitServerErrorDomain code:kGitServerNoSocketsAvailable userInfo:nil];
    ...
    if (kCFSocketSuccess != CFSocketSetAddress(_ipv4socket, (CFDataRef)address4)) {
        if (error) *error = [[NSError alloc] initWithDomain:GitServerErrorDomain code:kGitServerCouldNotBindToIPv4Address userInfo:nil];
        if (_ipv4socket) CFRelease(_ipv4socket);
        _ipv4socket = NULL;
        return NO;
    }
...
+ (NSString*) bonjourTypeFromIdentifier:(NSString*)identifier {
    ...
    return [NSString stringWithFormat:@"_%@._Git.", identifier];
}
